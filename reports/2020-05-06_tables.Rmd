...
all.tab2 <- get.tab1(df = filter(dta_ips, filler == 0), col.name = "Analytic cohort", table_engine = "xtable")
# For suicide cases in the ANALYTIC cohort
suicide.tab2 <- get.tab1(df = filter(dta_ips, suicide == 1, filler == 0), col.name = "Suicide cases", table_engine = "xtable")
# For overdose cases in the ANALYTIC cohort
poison.tab2 <- get.tab1(df = filter(dta_ips, poison == 1, filler == 0), col.name = "Fatal overdose cases", table_engine = "xtable")

tab2 <- cbind(all.tab2, suicide.tab2, poison.tab2)

# Get Table 1 for full FULL cohort
all.tab1 <- get.tab1(df = dta_full, col.name = "Full cohort", table_engine = "xtable")
# For suicide cases in the FULL cohort
suicide.tab1 <- get.tab1(df = filter(dta_full, suicide == 1), col.name = "Suicide cases", table_engine = "xtable")
# For overdose cases in the FULL cohort
poison.tab1 <- get.tab1(df = filter(dta_full, poison == 1), col.name = "Fatal overdose cases", table_engine = "xtable")
...
rownames(tab1) <- gsub("\\\\textbf\\{", "**", rownames(tab1))
rownames(tab1) <- gsub("\\}", "**", rownames(tab1))
pander(tab1, emphasize.rownames = F, missing = "", justify = c("lrrr"),
...
pander(tab2, emphasize.rownames = F, missing = "", justify = c("lrrr"),
